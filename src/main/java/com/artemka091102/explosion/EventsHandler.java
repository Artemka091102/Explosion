...
public class EventsHandler {
            ...
            if (!blocksPos.contains(GarbageBlockPos)) {
                ...
                if (fallBlocksNearCrater && flyBlocksNearCrater) {
                    if (canFlyThrough(world.getBlockState(getNearBlock(GarbageBlockPos, i)))) {
                        ...
                        fallBlock(world, GarbageBlockPos, calculateVelocity(GarbageBlockPos, center,
                                world.getBlockState(GarbageBlockPos).getHarvestLevel() + 2));
                    } else {
                        degradeBlock(world, GarbageBlockPos, degradedBlocksPos, 0.3F);
                    }
                }
    ...
    private static void degradeBlock(World world, BlockPos blockPos, HashSet<BlockPos> degradedBlocksPos,
                                     float chance) {
        ...
        BlockState newBlockState =
                crackedDictionary.get(world.getBlockState(blockPos).getBlock().getRegistryName().toString());
    ...
    private static void fallBlock(World world, BlockPos blockPos, Vec3d velocity) {
        ...
        FallingBlockEntity FBE = new FallingBlockEntity(world, blockPos.getX(), blockPos.getY(), blockPos.getZ(),
    ...
    public static void onCommonSetup(FMLLoadCompleteEvent event) {
        ...
        putToDictionary("wildnature:conglomerate_stairs_bricks_chiseled", "wildnature" +
                ":conglomerate_stairs_bricks_cracked");
